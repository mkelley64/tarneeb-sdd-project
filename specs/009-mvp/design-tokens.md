# Tarneeb iOS MVP 009 Design Tokens
...
| `color.station.outline.active` | `#FFB300` | Active bidding-turn or trick-turn player highlight; must match `color.button.newGame.background` |
...
- During active bidding or trick play, the current acting player may use
  `color.station.outline.active` plus a small active-turn indicator.
...
| `color.postBiddingSummary.team.text` | `#FFFFFF` | High-bidding player value text for the South post-bidding `Contract` suit-setting panel and final contract box |
| `color.postBiddingSummary.bid.text` | `#FFB300` | High bid value text for the South post-bidding `Contract` suit-setting panel, active station metadata, and final contract box; must match `color.button.newGame.background` |
...
- The final summary must display as an outside-table upper-left two-line contract box: `[Player]: [High Bid]` and `Tarneeb: [Suit chip]`. Station bid chips disappear after bidding completes.
...
- The high bid value, where shown in the station bid chip while bidding is active, South `Contract` panel, or final contract box, uses the same amber as the current-highest bid and `New Game` button.
...
| Token | Value | Usage |
|---|---|---|
| `color.trickPlay.slot.background` | `#0D2E21` | Seat-matched center played-card slot background during trick play |
| `color.trickPlay.slot.border` | `#E8DFC8` | Subtle border for empty and occupied played-card slots |
| `color.trickPlay.activeSeat.outline` | `#FFB300` | Active trick-turn station outline; must match `color.station.outline.active` and `color.button.newGame.background` |
| `color.trickPlay.legalCard.outline` | `#FFB300` | South legal-card affordance while South is the active trick player |
| `color.trickPlay.winnerHighlight.border` | `#FFB300` | Brief resolved-winner highlight before a completed trick clears |
| `color.trickPlay.count.background` | `#123A2A` | Compact trick-count spot background once trick play starts |
| `color.trickPlay.count.text` | `#FFFFFF` | Compact trick-count value text |

### Trick Play Surface Usage

- Before trick play, center slots are visible only as quiet felt details and
  should use low opacity.
- During trick play, the four center slots become active landing positions for
  South, East, North, and West played cards.
- Legal South card affordance is shown only while South is the active trick
  player; illegal or unavailable cards should avoid strong highlight.
- Played cards use their normal card face/back assets in the matching center
  slot; unplayed simulated cards remain hidden in station stacks.
- The compact trick-count spot is an individual player progress cue, not a
  partnership total or scoring surface.
...
- When a simulated player passes because their partner is currently highest, the
  ...
  or special visual affordance is introduced for MVP 009.
...
| `color.tableTitle.text` | `#BBAA7E` | Muted felt-stamped text color for the `طرنيب` title on the card table |
| `effect.tableTitle.text.opacity` | `0.72` | Subdued text opacity for the `طرنيب` title on the card table |
| `effect.tableTitle.shadow.color` | `#000000` | Dark inset title shadow color |
| `effect.tableTitle.shadow.opacity` | `0.38` | Dark inset title shadow opacity |
| `effect.tableTitle.shadow.blurRadius` | `1.2` | Tight inset title shadow blur radius |
| `effect.tableTitle.shadow.offset.y` | `1` | Downward shadow offset for the embossed felt effect |
| `effect.tableTitle.highlight.color` | `#F2E8CE` | Faint top highlight color for the embossed felt effect |
| `effect.tableTitle.highlight.opacity` | `0.18` | Faint top highlight opacity |
| `effect.tableTitle.highlight.blurRadius` | `0.8` | Tight top highlight blur radius |
| `effect.tableTitle.highlight.offset.y` | `-1` | Upward highlight offset for the embossed felt effect |
...
| `effect.trickPlay.slot.background.opacity` | `0.34` | Opacity for active center played-card slot fill during trick play |
| `effect.trickPlay.slot.border.opacity` | `0.42` | Opacity for center played-card slot borders |
| `effect.trickPlay.playedCard.shadow.opacity` | `0.28` | Shadow opacity for played cards resting in center slots |
| `effect.trickPlay.legalCard.outline.opacity` | `0.95` | Opacity for legal South card affordance during South's trick turn |
| `effect.trickPlay.unavailableSouthCard.opacity` | `0.55` | Opacity for South cards that cannot currently be played |
| `effect.trickPlay.winnerHighlight.opacity` | `0.30` | Opacity for brief winning-card/station emphasis before trick clear |
| `effect.trickPlay.count.background.opacity` | `0.68` | Opacity for the compact station trick-count badge so it reads as a badge rather than stray label text |
...
- The title shadow and highlight are part of the specified embossed felt title
  style.
...
- Trick-play slot and legal-card affordance effects should make the active
  player and legal choices clear without obscuring card faces.
...
- The card table title should remain readable against the table surface while
  feeling inset into the felt through the specified shadow/highlight treatment;
  the undealt deck is positioned in the dealer station and should not cover the title.
...
- Trick played-card slot width: `42pt`
- Trick played-card slot height: `58pt`
- Trick played-card slot gap: `6pt`
- Trick-count spot height: `18pt`

## Bidding and Trick Layout Tokens
...
| `layout.postBiddingSummary.outsideTableHorizontalOffsetRatio` | `0.62` | Horizontal offset ratio that moves the contract box above-left and off the table felt |
| `layout.postBiddingSummary.outsideTableVerticalOffsetRatio` | `0.70` | Vertical offset ratio that moves the contract box above-left and off the table felt |
| `layout.trickPlay.slot.width` | `42pt` | Width for each seat-matched center played-card slot |
| `layout.trickPlay.slot.height` | `58pt` | Height for each seat-matched center played-card slot |
| `layout.trickPlay.slot.gap` | `6pt` | Minimum spacing between center played-card slots and the table title/ribbon |
| `layout.trickPlay.slot.cornerRadius` | `6pt` | Corner radius for empty played-card slot outlines; card artwork keeps its own native corner treatment |
| `layout.trickPlay.counter.height` | `18pt` | Height for compact station trick-count spots once trick play starts |
| `layout.trickPlay.counter.minimumWidth` | `34pt` | Minimum width for compact station trick-count spots |
| `layout.trickPlay.counter.headerOffset` | `4pt` | Reserved header-relative trick-counter offset; MVP 009 visible station counters use the station bottom edge |
| `layout.trickPlay.counter.stationEdgeOffset` | `8pt` | Offset used to hang each station's trick counter from the bottom border so it does not cover player names or cards |

### Trick Play Layout Usage

- Center slot layout must preserve the clear play area prepared in the table
  center; bidding and summary UI should not claim that space.
- Played cards should fit inside the four seat-matched slots without covering
  the compact contract box.
- East and West played cards should use upright/vertical slot orientation in
  the center play area so their ranks and suits remain readable.
- The trick-count spot reserves station space for progress without creating a
  scoreboard or shifting station names. It displays the individual player's
  won-trick count, remains visually separate from player names and card backs,
  and every station, including South, places the count badge on the center of
  the station bottom edge rather than on the cards.
...
| `animation.trick.playedCard.flight.duration` | `0.30s` | Duration for a played card traveling from a station or hand to its matching center play-area slot |
| `animation.trick.clear.pause.duration` | `0.75s` | Readable pause after the fourth played card is visible and before the completed trick clears |
| `animation.trick.clear.fade.duration` | `0.20s` | Fade duration for clearing completed trick cards from the center slots |
...
### Trick-Play Motion Usage

- Played cards use `animation.trick.playedCard.flight.duration` when traveling
  from South, West, North, or East into the corresponding center slot.
- The motion should be quick and readable so it answers which seat acted without
  making automated trick play feel slow.
- After the fourth card arrives, keep all four cards visible for
  `animation.trick.clear.pause.duration`, then clear them using
  `animation.trick.clear.fade.duration` before the next trick begins.
- South illegal-card attempts should not use the played-card flight duration
  because no card is accepted into the trick.
...
- An all-pass automatic redeal should reuse the existing `animation.deal.*` timing tokens for its fresh deal animation; no separate automatic-redeal timing tokens are introduced for MVP 009.
...
struct GameColors {
    ...
    static let tableTitle = Color(hex: "#BBAA7E").opacity(0.72)
    static let tableTitleShadow = Color.black.opacity(0.38)
    static let tableTitleHighlight = Color(hex: "#F2E8CE").opacity(0.18)
    ...
    static let trickPlaySlotBackground = Color(hex: "#0D2E21")
    static let trickPlaySlotBorder = Color(hex: "#E8DFC8")
    static let trickPlayActiveSeatOutline = Color(hex: "#FFB300")
    static let trickPlayLegalCardOutline = Color(hex: "#FFB300")
    static let trickPlayWinnerHighlightBorder = Color(hex: "#FFB300")
    static let trickPlayCountBackground = Color(hex: "#123A2A")
    static let trickPlayCountText = Color(hex: "#FFFFFF")
    ...
    static let trickPlaySlotBackgroundOpacity = 0.34
    static let trickPlaySlotBorderOpacity = 0.42
    static let trickPlayPlayedCardShadowOpacity = 0.28
    static let trickPlayLegalCardOutlineOpacity = 0.95
    static let trickPlayUnavailableSouthCardOpacity = 0.55
    static let trickPlayWinnerHighlightOpacity = 0.30
...
struct TrickAnimationTiming {
    ...
    static let clearPauseDuration = 0.75
    static let clearFadeDuration = 0.20
}
...
struct BidAreaLayout {
    ...
    static let selectorMinimumWidth = 32
    static let selectorOptionGap = 2
...
struct TrickPlayLayout {
    static let slotWidth = 42
    static let slotHeight = 58
    static let slotGap = 6
    static let slotCornerRadius = 6
    static let countHeight = 18
    static let countMinimumWidth = 34
}
```
