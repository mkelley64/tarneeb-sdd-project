# Tarneeb iOS MVP 009 Requirements

## 1. Purpose

Build an iOS application MVP for the card game Tarneeb that allows one human player to start a game by dealing cards, enter a simple bidding round, establish the Tarneeb suit, and play the resulting hand through 13 legal tricks. The other three players are simulated seats. After a valid deal, the bidding round resolves with either a numeric high bid or an all-pass redeal. When a numeric high bid exists, the bidding area fades away, any required South Tarneeb suit selection is resolved, station bid chips disappear, a compact outside-table upper-left post-bidding contract box displays the high bidder, bid value, and preferred Tarneeb suit symbol, and trick play begins with the high bidder leading the first trick. South plays cards by dragging them from the South station to the table or by double-tapping a legal card. Simulated players play legal cards automatically. Played cards move to the table, each station's card stack or hand depletes, each trick winner is determined, and the winner leads the next trick until all 13 tricks are complete. Scoring, full game-over handling, multiplayer, persistence, and advanced trick-play AI are not included.

This MVP 009 revision carries forward the MVP 002 card readability and station sizing requirements, the MVP 003 portrait card-table changes, and the MVP 004 dealer selection, dealer indication, dealer-station squared undealt deck placement, counterclockwise dealer rotation, and sequential deal animation. It adds a simplified Tarneeb bidding round after the deal, including player stations that act as information surfaces for each seat's current bid/pass state, a bordered `Bidding` action area under the South player station, sequential counterclockwise bidding, a South player bid-chip selector when it is South's turn, a South `Bid` button that remains visible during in-progress bidding and is disabled unless it is South's turn with a valid submission, a post-bidding South Tarneeb suit-setting panel with suit chips and a `Set` button only when South has the numeric high bid, automated simulated bid choices based on each simulated player's dealt hand, the current bidding context, and Phase 3 personality adjustments applied to the normal hand-strength estimate, delayed South-card reveal so South's card faces remain hidden until all hands are dealt, South's received stack remains visible as fanned card backs until reveal time, and South's cards then flip face-up left-to-right over 1.5 seconds using xCards face artwork with native card corners and complete scale variants, a one-second fade from the bidding area to either a post-bidding summary for numeric high bids, a South suit-setting panel when South's high bid still needs a suit, or an automatic redeal for all-pass bidding rounds, console logging of each player's hand after each completed deal, and a basic legal trick-play phase after numeric bidding completes.
...
- After Tarneeb is chosen, the high bidder leads the first trick.
- Each trick consists of one card from each player, played counterclockwise.
- A player must follow the led suit if they have at least one card of that suit.
- If a player is void in the led suit, they may play any card, including a Tarneeb/trump card.
- If any Tarneeb cards are played in a trick, the highest Tarneeb card wins; otherwise the highest card in the led suit wins.
- The winner of a trick leads the next trick.
- A hand contains 13 tricks because each player starts with 13 cards.
...
- The `طرنيب` title should use the tokenized table-title style defined in `design-tokens.md`, including the specified Arabic rounded font, 26pt size, tracking range, muted text color/opacity, and embossed felt shadow/highlight treatment so it reads as part of the table surface.
...
- Once bidding starts, the `D` pill may disappear because dealer identity remains available through game state/test metadata and does not affect other MVP 009 gameplay.
...
### 3.6 MVP 009 Bidding Requirement Changes
...
- When the bidding round is complete, station bid chips should disappear so they do not conflict with trick counters.
- When the bidding round is complete with a numeric high bid, a compact post-bidding contract box should appear above and to the left of the table after the fade-out completes.
- The final post-bidding contract box should show `[Player]: [High Bid]` on the first line and `Tarneeb: [Suit chip]` on the second line. The suit symbol should use a compact white chip, black for Spades/Clubs, and red for Hearts/Diamonds.
...
- This MVP requires basic card-strength-based bid recommendations for simulated players, post-bidding South Tarneeb suit setting only when South wins the bidding, a displayed preferred Tarneeb suit summary after bidding, and legal trick play for the accepted contract. It does not require changing the Tarneeb suit after the final summary, scoring, learning-based AI, opponent modeling, or Monte Carlo-style search.
...
### 3.8 MVP 009 Design Improvement Requirements
...
- The post-bidding South suit-setting panel should read as a single `Contract` result surface with player, bid, Tarneeb suit chips, and a `Set` button. The final post-bidding summary should be a compact outside-table upper-left two-line contract box with the actual high bidder, bid value, and Tarneeb suit chip.
...
- During trick play, the center table play area should become the active trick surface with four seat-matched played-card slots.
- Player stations should continue to act as information surfaces during trick play, showing the seat label, remaining card count or depleted stack state, the winning bid chip where applicable, active-player emphasis, and the reserved trick counter spot once tricks are being won. The visible station trick counter must show that individual player's won-trick count, not the partnership total. The trick counter must sit in its own separated badge on the center of each station's bottom border, including South, so it does not visually attach to, overlap, or crowd the player name or card backs.
- The compact outside-table upper-left contract box should remain available during trick play so the trump suit is always visible without claiming the center play area or table felt.

### 3.9 MVP 009 Trick-Play Requirement Changes

- After bidding completes with a numeric high bid and the Tarneeb suit is known, the app should start a trick-play phase instead of ending at the post-bidding summary.
- The high bidder is the declarer and should lead the first trick.
- Trick-play turns should proceed counterclockwise.
- Each trick should collect exactly one played card from South, East, North, and West.
- The first card played in a trick establishes the led suit for that trick.
- Each player must follow the led suit if they have at least one card of that suit.
- If a player has no cards in the led suit, that player may play any remaining card, including a Tarneeb card.
- A played Tarneeb card beats every non-Tarneeb card in the trick.
- If one or more Tarneeb cards are played, the highest-ranked Tarneeb card wins the trick.
- If no Tarneeb card is played, the highest-ranked card in the led suit wins the trick.
- Card rank order for trick comparison should be Ace, King, Queen, Jack, 10, 9, 8, 7, 6, 5, 4, 3, 2.
- The trick winner should collect the trick for their partnership and lead the next trick.
- Trick play should continue until all 13 tricks are completed and every player has zero cards remaining.
- South should play a valid card by dragging it from the South station to the center table play area or by double-tapping it.
- If South attempts to play an invalid card, the card should remain in South's hand and the turn should not advance.
- When any player plays a card, that card should animate or move into the seat-matched center play-area slot.
- When a card is played, it should be removed from that player's remaining hand or hidden stack, causing the station display to deplete from 13 down to 0 across the hand.
- East, North, and West should play automatically on their turns using a simple legal-card strategy; advanced trick-play AI is out of scope.
- The app should track both individual player trick counts and North-South/East-West trick counts internally during the hand. Compact station trick counters display individual player trick counts only; scoring the contract is out of scope for MVP 009.
- East and West played cards in the center table play area should remain upright/vertical rather than rotated sideways.
...
- Display a compact outside-table upper-left post-bidding contract box after the `Bidding` area fade completes and any required South suit-setting step is complete, showing the actual high bidder, high bid value, `Tarneeb` label, and preferred suit symbol when a numeric high bid exists.
- Start trick play after numeric bidding completes and the Tarneeb suit is known.
- Use the high bidder as the declarer and first trick leader.
- Play 13 tricks with one card from each player per trick.
- Enforce Tarneeb follow-suit rules for South and simulated players.
- Determine each trick winner using Tarneeb/trump priority and led-suit high-card ranking.
- Advance the next trick leader to the previous trick winner.
- Allow South to play a legal card by dragging it from the South station to the table or by double-tapping it.
- Animate played cards from each station or hand into the seat-matched center play-area slot.
- Remove each played card from that player's remaining hand or hidden-card stack.
- Deplete South's visible hand and East/North/West hidden stacks from 13 cards to 0 cards across trick play.
- Track North-South and East-West trick counts internally while scoring remains out of scope.
...
- Scoring.
- Ending a full game.
- Advanced trick-play AI, card counting, or optimal play search for simulated players.
- Online multiplayer.
- Local multiplayer.
- User accounts.
- Saved games.
...
- Given the deal is complete and bidding has not yet produced a numeric contract, then no trick-play, scoring, or game-over controls are shown; the only Tarneeb-suit control allowed before trick play is the post-bidding South suit-setting panel shown when South has the numeric high bid.
...
- Given the circular card table is displayed, then its center area reserves four subtle seat-matched target slots for trick-play cards.
- Given the center target slots are displayed before trick play starts, then they are visually quiet and non-interactive.
- Given trick play starts, then the center target slots become the landing positions for played cards from South, East, North, and West.
...
- Given the bidding round reaches any terminal state, then player-station bid chips disappear so they do not conflict with trick counters.
- Given the bidding round reaches a terminal state with a numeric highest bid, then a compact post-bidding contract box appears above and to the left of the table after the `Bidding` area fade-out completes.
- Given the post-bidding contract box appears, then it uses two lines: `[Player]: [High Bid]` on the first line and `Tarneeb: [Suit chip]` on the second line.
- Given the post-bidding contract box appears, then `[Player]` is the actual high-bidding player, not the high bidder's team name.
- Given the post-bidding contract box appears, then it displays the high bidder's preferred suit symbol in a compact white chip.
...
- Given the final Tarneeb suit is known after numeric bidding, then the app is eligible to start trick play.
...
- Given bidding completes with any numeric high bid, then the app uses the stored preferred Tarneeb suit as the trump suit for trick play after any required South post-bidding `Set` action completes.
...
As a human player, I want the completed bidding area and station bids to clear away, then show a compact contract box with the high bidder, bid, and Tarneeb suit.
...
- Given the bidding round reaches any terminal state, then player-station bid chips disappear.
- Given the one-second fade-out completes and a numeric high bid exists, then a compact post-bidding contract box is displayed above and to the left of the table.
- Given the post-bidding contract box displays, then its first line displays `[Player]: [High Bid]` using the actual high-bidding player and bid value.
- Given the post-bidding contract box displays, then its second line displays `Tarneeb: [Suit chip]` with the preferred suit symbol for the highest bidder.
...
As a human player, I want to play the hand after bidding so the game proceeds through actual Tarneeb tricks.

#### Acceptance Criteria

- Given bidding completes with a numeric high bid and the Tarneeb suit is known, then the app starts a trick-play phase.
- Given the trick-play phase starts, then the high bidder is the declarer and leads the first trick.
- Given a trick is active, then the current turn belongs to exactly one seat.
- Given a card is played, then the next turn advances counterclockwise to the next seat until four cards have been played to the trick.
- Given a trick has no played cards yet, then the active player may lead any remaining card.
- Given the first card in a trick is played, then that card's suit becomes the led suit.
- Given a player has at least one card in the led suit, then that player may play only a card in the led suit.
- Given a player has no card in the led suit, then that player may play any remaining card, including a Tarneeb card.
- Given South is the active player and at least one South card is legal, then South can play a legal card by dragging it from the South station into the table play area.
- Given South is the active player and at least one South card is legal, then South can play a legal card by double-tapping it.
- Given South drags a legal card into the table play area, then the card moves to South's center play-area slot and is removed from South's hand.
- Given South double-taps a legal card, then the card moves to South's center play-area slot and is removed from South's hand.
- Given South drags or double-taps an illegal card, then the card remains in South's hand, no played-card slot changes, and the turn does not advance.
- Given it is not South's turn, then South card drag and double-tap gestures must not play a card.
- Given East, North, or West is the active player, then the app automatically selects and plays one legal card from that simulated player's remaining hidden hand.
- Given a simulated player plays a card, then the played card moves to that player's center play-area slot and the simulated station's hidden stack count decreases by one.
- Given any player plays a card, then that player's remaining hand count decreases by exactly one.
- Given a played card moves to the table, then it is displayed in the seat-matched center play-area slot until the trick is cleared.
- Given East or West plays a card to the center table play area, then that played card is displayed upright/vertical rather than sideways.
- Given four cards have been played to a trick, then the app determines the trick winner before starting the next trick.
- Given at least one Tarneeb card was played in a trick, then the highest-ranked Tarneeb card wins the trick.
- Given no Tarneeb card was played in a trick, then the highest-ranked card of the led suit wins the trick.
- Given the app compares ranks for trick resolution, then rank order is Ace, King, Queen, Jack, 10, 9, 8, 7, 6, 5, 4, 3, 2.
- Given a trick winner is determined, then the winner's partnership trick count increases by one.
- Given a trick winner is determined, then only the winning player's visible station trick counter increases by one; the winner's partner station counter does not increase from that trick.
- Given a trick winner is determined and fewer than 13 tricks have been completed, then the trick clears after a short readable pause and the winner leads the next trick.
- Given a trick winner is determined for the 13th trick, then the hand's trick-play phase is complete and all player hands have zero cards remaining.
- Given trick play is complete, then no additional card can be played until the user starts a replacement deal or taps `New Game`.
- Given a replacement deal starts after trick play, then previous played cards, trick counts, active trick state, and remaining hands are cleared.
- Given `New Game` is tapped during trick play or after trick play completes, then previous played cards, trick counts, active trick state, and remaining hands are cleared.
...
For trick resolution, ranks compare from high to low as `A`, `K`, `Q`, `J`, `10`, `9`, `8`, `7`, `6`, `5`, `4`, `3`, `2`.
...
- `phase`: one of `notStarted`, `dealt`, `trickPlay`, `handComplete`
...
- `trickPlayState`: optional trick-play data once a numeric contract has a known Tarneeb suit
- `currentTrickIndex`: a value from 1 through 13 while trick play is active
- `currentTrickLeaderSeat`: the seat that leads the active trick
- `currentTrickTurnSeat`: the active seat expected to play the next card
- `currentTrickLedSuit`: empty before the first card in a trick, then the suit of the first played card
- `currentTrickPlayedCards`: ordered played-card records for the active trick, containing seat and card
- `completedTricks`: ordered completed trick records for the current hand
- `individualTrickCounts`: completed trick totals keyed by winning player seat for the current hand; these drive the visible station trick counters
- `teamTrickCounts`: North-South and East-West completed trick totals for the current hand
- `remainingHandCounts`: the number of unplayed cards remaining for each seat
...
Each trick-play state record should contain:

- `declarerSeat`: the high bidder who leads the first trick
- `tarneebSuit`: the final trump/Tarneeb suit for the hand
- `currentTrickIndex`: a value from 1 through 13 while trick play is active
- `currentTrickLeaderSeat`: the seat that leads the active trick
- `currentTrickTurnSeat`: the active seat expected to play the next card
- `currentTrickLedSuit`: empty until the first card of a trick is played, then the led suit
- `currentTrickPlayedCards`: ordered played-card records containing seat and card
- `completedTricks`: ordered completed trick records containing the four played cards, led suit, winning seat, winning partnership, and Tarneeb suit
- `individualTrickCounts`: completed trick totals keyed by winning player seat
- `teamTrickCounts`: completed trick totals for North-South and East-West
- `remainingHandCounts`: the number of unplayed cards remaining for each seat

No post-summary editable Tarneeb suit selection, scoring, or game-over phase should be implemented for MVP 009. The only interactive Tarneeb suit control in MVP 009 is South's post-bidding suit-setting panel when South is the numeric high bidder.
...
- The circular table center must reserve a clear play area with four subtle target slots corresponding to South, West, North, and East.
- Before trick play starts, the play area target slots must be visually quiet and non-interactive.
- During trick play, played cards must land in the seat-matched target slot for the player who played the card.
- During trick play, East and West target slots must display played cards upright/vertical for readability.
...
- After the `Bidding` area is removed, a compact post-bidding contract box must appear above and to the left of the table when a numeric high bid exists.
- The post-bidding contract box must display `[Player]: [High Bid]` on the first line and `Tarneeb: [Suit chip]` on the second line.
- Player-station bid chips must disappear once bidding completes so they do not conflict with trick counters.
...
- Simulated bid values must consider basic card strength and partnership context, but do not need to consider score, opponent modeling, or trick-play simulation in MVP 009.
- Accepted simulated numeric bids must store the preferred trump/Tarneeb suit alongside the resolved bid value and retain confidence metadata internally so the high bidder's preferred suit can be shown in the post-bidding summary, without starting post-bidding interactive Tarneeb suit selection in MVP 009.
...
- The post-bidding summary data and visible contract box must retain and display the actual highest bidder and bid value.
- The visible post-bidding contract box must use the highest bidder's preferred trump/Tarneeb suit stored alongside the accepted bid to derive the suit symbol.
- If no stored preferred Tarneeb suit exists for a numeric highest bid, the state is invalid for MVP 009 because every accepted numeric bid must store a preferred suit.
...
Auction and trick-play limitations for MVP 009:

- The app lets South choose a Tarneeb suit only in the post-bidding suit-setting panel when South is the numeric high bidder.
- The app proceeds into legal trick play after numeric bidding and Tarneeb suit selection, but it does not score the contract or show a full game-over state.

### 6.10 Trick-Play Behavior

After numeric bidding completes and the final Tarneeb suit is known, the implementation must play the hand through 13 legal Tarneeb tricks.

Trick-play setup:

- The high bidder becomes the declarer.
- The declarer leads the first trick.
- The Tarneeb suit is read from the accepted high bid record after any required South `Set` action.
- Trick play starts with `currentTrickIndex = 1`, empty `currentTrickPlayedCards`, empty `currentTrickLedSuit`, and the declarer as both `currentTrickLeaderSeat` and `currentTrickTurnSeat`.
- Each player's remaining hand count starts at 13.
- Individual player trick counts and North-South/East-West trick counts start at 0.

Legal-card rules:

- If a player leads a trick, every remaining card in that player's hand is legal.
- If a led suit exists and the active player has one or more cards in that suit, only cards in the led suit are legal.
- If a led suit exists and the active player has no cards in that suit, every remaining card in that player's hand is legal.
- South card gestures must be accepted only when South is the active trick-play turn and the selected card is legal.
- Illegal South drag or double-tap attempts must leave all trick state unchanged.

South play behavior:

- South can play a legal card by dragging it from the South station to the center table play area.
- South can play a legal card by double-tapping it.
- A valid South play removes that card from South's visible hand, moves it to South's center play-area slot, and advances the turn counterclockwise when the trick is not complete.
- If South has legal cards in the led suit, non-led-suit cards should be visibly unavailable or should snap back when attempted.

Simulated play behavior:

- East, North, and West play automatically when their trick-play turn becomes active.
- Simulated players must always choose from legal cards only.
- MVP 009 may use a simple deterministic legal-card strategy, such as following suit with a lowest legal card and otherwise discarding a lowest legal card, as long as it obeys Tarneeb rules.
- A simulated play removes the selected card from that simulated player's hidden hand, moves the card to that player's center play-area slot, and decreases the station's hidden stack count by one.

Trick resolution:

- A trick is complete when four played-card records exist for the active trick.
- The first played card defines the led suit.
- If at least one played card has the Tarneeb suit, the highest Tarneeb card wins the trick.
- If no played card has the Tarneeb suit, the highest card in the led suit wins the trick.
- Rank order for comparison is Ace, King, Queen, Jack, 10, 9, 8, 7, 6, 5, 4, 3, 2.
- The winning seat's partnership trick count increases by one.
- The winning seat's individual trick count increases by one, and the winning seat's partner individual trick count remains unchanged.
- The completed trick is appended to `completedTricks`.
- After a short readable pause, the table clears the four played cards.
- If fewer than 13 tricks are complete, the winning seat becomes the next `currentTrickLeaderSeat` and `currentTrickTurnSeat`.
- If 13 tricks are complete, the hand enters `handComplete`; all player remaining hand counts must be 0 and no further card play is accepted.

### 6.11 Automated Bid Recommendation Behavior
...
- The post-bidding contract box displays `[Player]: [High Bid]` and `Tarneeb: [Suit chip]`, with `♠`, `♣`, `♥`, or `♦` in a compact white chip using the proper black/red suit color.
- If South is the highest bidder, the post-bidding summary appears only after South selects a Tarneeb suit and taps `Set`.
- A trick-play area in the center of the table after numeric bidding completes and the Tarneeb suit is known.
- Four seat-matched center play-area slots that receive played cards from South, East, North, and West.
- Active-player emphasis during trick play so the user can tell whose turn it is to play.
- South card drag support from the South station to the table for legal South plays.
- South card double-tap support for legal South plays.
- Played cards displayed in the center play-area slots until the trick resolves and clears.
- South's visible hand depleting by one card after each valid South play.
- West, North, and East hidden-card stacks depleting by one card after each simulated play.
- A compact indication of trick progress or trick counts is allowed once trick play begins, but no scoring surface is shown.
...
- The app must be locked to portrait orientation for MVP 009.
...
- Trick legal-card validation, trick winner resolution, trick count updates, and turn advancement should be isolated from SwiftUI views.
...
- The post-bidding summary should be exposed in a way that UI tests can verify the visible high-bidding player, high bid value, Tarneeb label, suit symbol, and outside-table upper-left placement.
...
- Trick-play state should be exposed in a way that tests can verify current trick index, led suit, current turn seat, current trick played cards, legal South card set, completed trick winners, individual station trick counts, team trick counts, and remaining hand counts.
- South drag and double-tap play paths should be testable through stable accessibility identifiers or presentation metadata where practical.
...
- The app must never accept a trick-play card from a player whose turn is not active.
- The app must never accept an illegal off-suit card from a player who can follow the led suit.
- The app must never allow a card to be played more than once.
- A completed trick must always contain exactly four cards, one from each seat.
- A completed hand must always contain exactly 13 completed tricks and zero remaining cards for every player.
...
- The central table, dealer-station squared deck stack, rounded-square stations, dealer pill, `Bidding` area, post-bidding summary, played-card slots, South `Bid` button, bottom controls, and status label must not overlap incoherently.
...
- If a replacement deal is requested during trick play or after trick play completes, previous played cards, trick counts, active trick state, and remaining hands must be cleared before the replacement deal is shown.
...
- If `New Game` is requested during trick play or after trick play completes, previous played cards, trick counts, active trick state, and remaining hands must be cleared with the rest of the dealt-state UI.
...
- If trick play starts without a numeric high bidder or without a known Tarneeb suit, the state is invalid.
- If a trick has no played cards, the active leader may play any remaining card.
- If a trick has a led suit and the active player has that suit, only that suit is legal.
- If a trick has a led suit and the active player is void in that suit, any remaining card is legal.
- If South attempts to play when it is not South's turn, the attempted card must remain in South's hand and no turn state may change.
- If a simulated player has a legal card available, the simulated play must select exactly one legal card and must not expose any other hidden cards.
- If a trick has fewer than four played cards, no trick winner should be recorded yet.
- If a trick has four played cards, exactly one trick winner must be recorded before the next trick begins.
...
- If bidding completes with a numeric high bid, player-station bid chips must disappear and the post-bidding contract box must display the correct high-bidding player label, numeric bid, and Tarneeb suit symbol.
...
- If a device is physically rotated, the app should remain in portrait orientation for MVP 009.
...
54. `testPostBiddingSummaryStoresHighBidderPlayerAndBid`
    - Resolve bidding with South, East, North, and West as highest bidder and verify the terminal summary metadata and visible contract box display the actual high-bidding player label and bid.
...
30. `testPostBiddingSummaryDisplaysContractBox`
    - Complete bidding with a numeric high bid and verify the outside-table upper-left contract box displays the expected high bidder, bid value, and `Tarneeb` suit symbol in a compact white chip with black/red suit coloring.
...
35. `testTrickPlayStartsAfterNumericContract`
    - Complete bidding with a numeric high bid and final Tarneeb suit, then verify trick play starts with the high bidder as the first leader and the active trick index set to 1.

35a. `testSouthCanPlayLegalCardByDragOrDoubleTap`
    - Put South on turn, play one legal South card by dragging it to the table, play another legal South card by double-tapping it in a separate setup, and verify each card moves to South's center slot, leaves South's hand, and advances turn.

35b. `testIllegalSouthCardDoesNotPlay`
    - Give South at least one card in the led suit, attempt to drag or double-tap a different suit card, and verify the card remains in South's hand and turn state does not change.

35c. `testTrickWinnerResolutionUsesTarneebAndLedSuit`
    - Resolve one trick containing Tarneeb cards and one trick without Tarneeb cards, verifying highest Tarneeb wins when present and otherwise highest led-suit card wins.

35d. `testPlayedCardsDepleteStationStacks`
    - Play cards from all four seats and verify South's visible hand count and East/North/West hidden stack counts each decrease by one.
...
34. Verify station bid chips disappear after bidding completes and the post-bidding contract box displays `[Player]: [High Bid]` and `Tarneeb: [Suit chip]`, with the preferred suit symbol in a compact white chip using black for Spades/Clubs and red for Hearts/Diamonds.
...
37. Verify trick play starts after a numeric contract has a known Tarneeb suit and the high bidder leads the first trick.
38. Verify South can play a legal card by dragging it from the South station to the table.
39. Verify South can play a legal card by double-tapping it.
40. Verify an illegal South card attempt snaps back or remains in hand and does not advance the turn.
41. Verify East, North, and West play legal cards automatically on their turns.
42. Verify each played card moves into the correct seat-matched center table slot.
43. Verify South's visible hand and the simulated players' hidden stacks deplete by one card per valid play.
44. Verify a trick containing Tarneeb cards is won by the highest Tarneeb card.
45. Verify a trick without Tarneeb cards is won by the highest card of the led suit.
46. Verify the trick winner leads the next trick and trick play stops after 13 completed tricks.
47. Verify each player's full hand is logged to the console after a completed deal, including any all-pass automatic redeal.
48. Verify tapping `New Game` after a deal or during trick play returns the app to the launch state with no dealt cards, no `Bidding` area, no post-bidding summary, no played cards, a selected dealer, a dealer pill beside the selected station's name, and the squared undealt deck stack visible inside the selected dealer station.

## 11. Definition of Done

The MVP 009 revision is complete when:
...
- When bidding ends with a numeric high bid by East, North, or West, station bid chips disappear and the final outside-table upper-left post-bidding contract box displays the actual high bidder, bid value, and `Tarneeb` with the preferred suit symbol.
...
- Trick play starts after a numeric contract has a known Tarneeb suit.
- The high bidder leads the first trick.
- South can play a legal card by dragging it to the table or double-tapping it.
- Invalid South play attempts leave the card in South's hand and do not advance the turn.
- East, North, and West automatically play legal cards on their turns.
- Played cards move into the table's seat-matched center slots.
- South's visible hand and the simulated players' hidden stacks deplete by one card per valid play.
- Trick winners are determined by Tarneeb/trump priority and led-suit rank order.
- Trick winners lead the next trick until all 13 tricks are complete.
- No post-summary editable Tarneeb suit selection, scoring, full game-over state, or advanced trick-play AI is implemented.
...
- Automated tests verify legal-card validation, South drag/double-tap play paths, simulated legal play, trick winner resolution, trick count updates, card depletion, and hand completion after 13 tricks.
...
- Exact undealt deck stack offset and rotation are not specified beyond a squared appearance while still representing 52 hidden cards; MVP 009 uses tokenized zero offset and zero rotation.
...
- It is not specified whether the 52-card undealt deck stack should animate away during deal. MVP 009 requires that it is present in the dealer station before deal, can decrement during the deal, and is absent after deal.
...
- Exact South Tarneeb suit selector control type is specified as a chip-style post-bidding selector for MVP 009, but exact chip dimensions beyond the tokenized minimums and compact summary symbol padding are not specified.
