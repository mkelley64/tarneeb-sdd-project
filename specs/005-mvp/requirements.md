# Tarneeb iOS MVP 005 Requirements

## 1. Purpose

Build an iOS application MVP for the card game Tarneeb that allows one human player to start a game by dealing cards and then enter a simple bidding round. The other three players are simulated seats. This MVP ends after a valid deal is completed, the bidding round resolves with a numeric high bid, the bidding area fades away, and a post-bidding summary displays the high-bidding team, bid value, and preferred Tarneeb suit symbol, while allowing the player to request a replacement deal or reset the screen to a launch state. If all four players pass before any numeric bid is accepted, the hand is abandoned and a new deal starts automatically with the dealer advanced counterclockwise to the previous dealer's right. South can choose a Tarneeb suit when committing a numeric bid. Post-bidding editable Trump/Tarneeb suit selection, trick play, scoring, multiplayer, persistence, and advanced gameplay AI beyond bidding personality are not included.

This 005 revision carries forward the MVP 002 card readability and station sizing requirements, the MVP 003 portrait card-table changes, and the MVP 004 dealer selection, dealer indication, very-centered squared undealt deck placement, counterclockwise dealer rotation, and sequential deal animation. It adds a simplified Tarneeb bidding round after the deal, including a bordered `Bidding` area under the South player station, a bid table for all four players, sequential counterclockwise bidding, a South player bid selector and Tarneeb suit selector when it is South's turn, a South `Bid` button that remains visible during in-progress bidding and is disabled unless it is South's turn with a valid submission, automated simulated bid choices based on each simulated player's dealt hand, the current bidding context, and Phase 3 personality adjustments applied to the normal hand-strength estimate, delayed South-card reveal so South's card faces remain hidden until all hands are dealt, South's received stack remains visible as fanned card backs until reveal time, and South's cards then flip face-up left-to-right over 1.5 seconds, a one-second fade from the bidding area to either a post-bidding summary for numeric high bids or an automatic redeal for all-pass bidding rounds, and console logging of each player's hand after each completed deal.
...
- In this MVP, South can choose a Tarneeb suit while committing a numeric South bid. The high bidder's preferred suit is displayed after bidding as a non-interactive Tarneeb summary label rather than as an editable post-bidding suit-selection step.
...
- When all 52 cards have been dealt, the center deck stack should be gone, South's station should expand to hold 13 card backs, and South's cards should flip face-up from left to right over 1.5 seconds.
- The completed deal state should be shown only after the South reveal flip finishes.

### 3.6 MVP 005 Bidding Requirement Changes
...
- The `Bidding` area should contain a table showing one bid value for each player.
- Each table value should begin as `--`.
...
- `Pass` and pending `--` states should not store a preferred trump/Tarneeb suit. Active South selection may hold a draft suit, but South's resolved bid should store a preferred suit only when South commits a numeric bid.
...
- While bidding is in progress, the South `Bid` button should always be visible in the `Bidding` area, positioned below the South player bid value.
- The South `Bid` button should be disabled unless it is South's turn and South has a valid submission.
- The `Bidding` area should extend vertically as necessary to contain the South `Bid` button below the South bid value without overlap.
- When it is South's turn to bid, South's table value should be replaced with a drop-down list and the South `Bid` button should become available for valid South submissions.
- The South drop-down list should include `Pass` and numeric values from the current legal minimum through `13`; the current legal minimum is `7` when no numeric bid exists, otherwise one more than the current highest numeric bid.
- When it is South's turn to bid, the `Bidding` area should also show a Tarneeb suit selector for South with `♠`, `♣`, `♥`, and `♦` options.
- The South Tarneeb suit selector should be required only when South's selected bid value is numeric.
- If South selects `Pass`, no South Tarneeb suit should be stored and the suit selector should be disabled, hidden, or ignored for that submission.
- If South selects a numeric bid, the South `Bid` button should not submit the bid until South has selected a Tarneeb suit.
- When South selects a bid value and taps `Bid`, the drop-down list and suit selector should be replaced by South's selected bid value, and the South `Bid` button should return to its disabled state unless bidding returns to South.
- When South commits a numeric bid, South's selected Tarneeb suit should be stored alongside South's accepted bid value.
...
- The post-bidding summary should show the high-bidding team as `East-West` or `North-South`, the high bid value, and a `Tarneeb` label containing the symbol for the high bidder's preferred suit.
...
- This MVP requires basic card-strength-based bid recommendations for simulated players, South bid-time Tarneeb suit selection for numeric South bids, and a displayed preferred Tarneeb suit summary after bidding, but does not require post-bidding editable Tarneeb suit selection, trick play, scoring, learning-based AI, opponent modeling, or Monte Carlo-style search.
...
- The preferred trump/Tarneeb suit should be stored for South from South's explicit bid-time suit selection when South commits a numeric bid.
- `Pass` and pending `--` bid states must not carry a preferred trump/Tarneeb suit. Active South selection may carry a draft suit selection, but no preferred suit is stored on South's resolved bid state unless South commits a numeric bid.
- The post-bidding summary must read the high bidder's stored preferred trump/Tarneeb suit from the bid record rather than relying only on transient recommendation metadata.
...
- Display a bid table inside the `Bidding` area with one value for South, East, North, and West.
- Initialize all bid table values to `--`.
...
- Allow the South player to choose from `Pass` and the currently legal numeric bids through `13` using a drop-down list when it is South's turn.
- Allow the South player to choose a Tarneeb suit from `♠`, `♣`, `♥`, and `♦` when committing a numeric bid.
- Store South's explicitly selected Tarneeb suit alongside South's numeric bid value when South commits a numeric bid.
...
- Enable the South `Bid` button only while it is South's turn with a valid submission: `Pass`, or a numeric bid paired with a selected Tarneeb suit; show it disabled otherwise.
...
- Display a post-bidding summary after the `Bidding` area fade completes, showing the high-bidding team, high bid value, and `Tarneeb` suit symbol when a numeric high bid exists.
- Replace the South drop-down and Tarneeb suit selector with the selected bid value after the human player taps `Bid`.
...
- Post-bidding editable Tarneeb suit selection or allowing the user to change the displayed preferred suit after a bid is committed.
...
- Given the fourth 13-card stack arrives, then the center deck stack is removed and the completed deal state is displayed only after South's left-to-right reveal flip completes.
...
- Given the deal is complete, then no trick-play, Tarneeb-suit, scoring, or game-over controls are shown.
...
- Given the bid table first appears, then each player entry shows `--`.
...
- Given it is South's turn to bid, then the South entry is replaced by a drop-down list and the South `Bid` button becomes available for valid South submissions.
- Given the South drop-down list is opened, then it contains `Pass` plus numeric values from the current legal minimum through `13`.
- Given no numeric bid exists, then the South drop-down numeric values start at `7`.
- Given a current highest numeric bid exists, then the South drop-down numeric values start at one more than the current highest numeric bid.
- Given it is South's turn to bid, then the `Bidding` area also shows a South Tarneeb suit selector with spades, clubs, hearts, and diamonds.
- Given South's selected bid value is numeric and no Tarneeb suit is selected, then the South `Bid` button remains disabled.
- Given South's selected bid value is numeric and a Tarneeb suit is selected, then the South `Bid` button is enabled.
- Given South's selected bid value is `Pass`, then the South `Bid` button is enabled without requiring a Tarneeb suit.
- Given the user selects `Pass` for South and taps `Bid`, then the South drop-down list and any South suit selector are replaced by `Pass`, no preferred trump/Tarneeb suit is stored for South, and the South `Bid` button becomes disabled unless bidding returns to South.
- Given the user selects a numeric South bid, selects a Tarneeb suit, and taps `Bid`, then the South drop-down list and suit selector are replaced by the selected bid value, the selected suit is stored alongside South's accepted bid state, and the South `Bid` button becomes disabled unless bidding returns to South.
- Given the South `Bid` button is positioned under the South bid value, then the `Bidding` area extends vertically as necessary so the button does not overlap the bid table, South hand, status label, or bottom controls.
- Given a player's bid value changes from `--`, the South drop-down, or any previous value, then the displayed value is applied with a one-second fade and color transition.
...
- Given the post-bidding summary appears, then it displays the high-bidding team as `East-West` when the highest bidder is East or West and `North-South` when the highest bidder is North or South.
- Given the post-bidding summary appears, then it displays the highest bid value.
- Given the post-bidding summary appears, then it displays a `Tarneeb` label with the high bidder's preferred suit symbol.
- Given the preferred suit is spades, clubs, hearts, or diamonds, then the summary symbol is `♠`, `♣`, `♥`, or `♦` respectively.
- Given the bidding round reaches a terminal state with no numeric highest bid, then the `Bidding` area still fades out and disappears, no high-bidding team or Tarneeb suit summary is displayed, and an automatic new deal begins.
- Given a replacement deal completes, then the fresh bidding round starts with all bid values reset to `--`.
- Given South is not actively choosing a numeric bid, then no interactive Tarneeb suit selector is enabled.
- Given the bidding round is complete, then the app does not show an interactive Tarneeb suit selector in this MVP.
...
- Given the bid generator estimates seven or more likely team tricks, then it recommends a bid from 7 through 13.
...
- Given the bid generator produces any recommendation, then it includes a confidence value in the range 0 through 1.
...
- Given a simulated player's personality profile is applied, then the personality starts from the normal estimated trick count produced by the hand-strength evaluator before legal-bid enforcement.
- Given a simulated player has a conservative, balanced, or aggressive personality, then the profile may adjust the normal estimate, bid threshold, or rounding behavior to make that player underbid, use the normal estimate, or overbid within controlled limits.
...
- Given a personality-adjusted recommendation is generated, then it must still be passed through the normal legal-bid rules and must not allow an equal-or-lower bid, a bid below 7, a bid above 13, or reentry after `Pass`.
...
- Given South commits a numeric bid, then the accepted South bid stores South's explicitly selected Tarneeb suit alongside the bid value, solely so the post-bidding summary can display a suit symbol if South is the highest bidder.
- Given bidding completes with a numeric high bid, then the app still stops at the `Bidding complete` state and does not use the preferred Tarneeb suit to start interactive suit selection or trick play.
...
As a human player, I want the completed bidding area to clear away and show the high-bidding team, bid, and Tarneeb suit symbol so I can understand the auction result.
...
- Given the highest bidder is East or West, then the post-bidding summary displays the high-bidding team as `East-West`.
- Given the highest bidder is North or South, then the post-bidding summary displays the high-bidding team as `North-South`.
- Given the post-bidding summary displays a high-bidding team, then it also displays the high bid value.
- Given the post-bidding summary displays a high-bidding team, then it also displays a `Tarneeb` label with the preferred suit symbol for the highest bidder.
...
- Given bidding completes with no numeric high bid, then no high-bidding team or Tarneeb suit summary is displayed.
...
- `southDraftTarneebSuit`: the South player's in-progress Tarneeb suit selection while South's turn is active, required only when the draft bid value is numeric
...
- `postBiddingSummary`: optional terminal summary containing the high-bidding team label, high bid value, and preferred Tarneeb suit symbol when a numeric high bid exists

Each bid state must be one of:

- `pending`, displayed as `--`
- `activeSouthSelection`, displayed as the South bid drop-down and South Tarneeb suit selector when it is South's turn
- `pass`, displayed as `Pass`
- a numeric bid from 7 through 13 with a stored preferred trump/Tarneeb suit

Only accepted numeric bid states may carry committed preferred trump/Tarneeb suit metadata. `pending` and `pass` states must not carry preferred trump/Tarneeb suit metadata. `activeSouthSelection` may carry a draft suit selection, but that draft suit is committed only when South submits a numeric bid.

Each accepted numeric bid state should contain:

- `value`: a numeric bid from 7 through 13
- `preferredTarneebSuit`: one of `spades`, `clubs`, `hearts`, or `diamonds`; for South numeric bids this comes from South's explicit suit selection, and for simulated numeric bids this comes from the automated evaluator
...
No post-bidding editable Tarneeb suit selection, trick-play, scoring, or game-over phase should be implemented for MVP 005. The only interactive Tarneeb suit control in MVP 005 is South's bid-time suit selector for numeric South bids.
...
- The South bid entry must become a drop-down list only while it is South's turn.
- The South Tarneeb suit selector must appear only while it is South's turn and must provide spades, clubs, hearts, and diamonds.
...
- The South `Bid` button must be enabled only while it is South's turn and South has a valid submission: `Pass`, or a numeric bid paired with a selected Tarneeb suit.
...
- The South drop-down list must contain `Pass` and the currently legal numeric bids through `13`.
...
- The post-bidding summary must display the high-bidding team, high bid value, and `Tarneeb` suit symbol.
...
- The layout must avoid overlapping labels, card faces, card backs, bid table content, status label text, and action buttons.
...
- When South's turn becomes active, South's table entry must be replaced with a drop-down list.
- When South's turn becomes active, the `Bidding` area must also show a Tarneeb suit selector for South.
...
- The South drop-down list must always include `Pass`.
- If no numeric bid exists, the South drop-down list must offer numeric bids from `7` through `13`.
- If a current highest numeric bid exists, the South drop-down list must offer numeric bids from one more than the current highest bid through `13`.
- The South Tarneeb suit selector must offer spades, clubs, hearts, and diamonds, displayed using the corresponding suit symbols or labels.
- If South selects `Pass`, the selected Tarneeb suit must not be required and must not be stored when `Pass` is submitted.
- If South selects a numeric bid, the South `Bid` button must remain disabled until South selects a Tarneeb suit.
- If South selects `Pass` and taps `Bid`, South's table entry must become `Pass`.
- If South selects a numeric bid, selects a Tarneeb suit, and taps `Bid`, South's table entry must become that numeric bid, South's selected suit must be stored alongside the bid value, and the current highest bid must update to South.
- After South taps `Bid`, the South drop-down list and Tarneeb suit selector must be replaced by the selected bid value and the `Bid` button must become disabled unless bidding returns to South.
...
- Simulated bid values must consider basic card strength and partnership context, but do not need to consider score, opponent modeling, or trick-play simulation in MVP 005.
- Accepted simulated numeric bids must store the preferred trump/Tarneeb suit alongside the resolved bid value and retain confidence metadata internally so the high bidder's preferred suit can be shown in the post-bidding summary, without starting post-bidding interactive Tarneeb suit selection in MVP 005.
...
- If no stored preferred Tarneeb suit exists for a numeric highest bid, the state is invalid for MVP 005 because every accepted numeric bid must store a preferred suit.
...
Auction limitations for MVP 005:
...
The preferred suit metadata must be stored alongside any accepted numeric bid. If South makes a numeric bid, the app must store South's explicitly selected Tarneeb suit with South's accepted bid for the post-bidding summary. Simulated numeric bids continue to use evaluator-generated preferred suit metadata. Bid states that resolve to `Pass` must not retain a preferred suit.
...
- A bid table showing South, East, North, and West.
- Bid table values initialized as `--`.
- A South bid drop-down list containing `Pass` and the currently legal numeric bids through `13` only while it is South's turn.
- A South Tarneeb suit selector containing `♠`, `♣`, `♥`, and `♦` only while it is South's turn.
...
- The South `Bid` button enabled only while it is South's turn and South has selected either `Pass` or both a numeric bid and a Tarneeb suit.
...
- The post-bidding summary displays `East-West` or `North-South`, the high bid value, and a `Tarneeb` label with `♠`, `♣`, `♥`, or `♦`.
- If South is the highest bidder, the post-bidding summary uses South's selected Tarneeb suit.
...
- Interactive Trump/Tarneeb suit selectors except South's bid-time Tarneeb suit selector for numeric South bids.
- Play-card controls.
- Trick area.
- Scoreboard.
- Game-over state.
...
- The app must be locked to portrait orientation for MVP 005.
...
- Bid recommendation generation, allowed bid values, bidding turn order, highest-bid tracking, South bid and Tarneeb suit selection state, and bidding completion rules should be testable independently of SwiftUI where practical.
...
- The `Bidding` area, bid table, South drop-down, South Tarneeb suit selector, and South `Bid` button should be exposed in a way that UI tests can verify visibility, allowed values, enabled/disabled active-turn behavior, selected bid and suit updates, simulated-bid timing, fade/color timing, terminal fade-out, and terminal button disappearance.
- The post-bidding summary should be exposed in a way that UI tests can verify the high-bidding team, bid value, and Tarneeb suit symbol.
...
- South numeric bid submission must never commit without a selected Tarneeb suit.
- South `Pass` submission must never store a preferred Tarneeb suit.
- A post-bidding summary with a high-bidding team must never appear unless a numeric high bid exists.
...
- The central table, very-centered squared deck stack, rounded-square stations, dealer badge, `Bidding` area, post-bidding summary, South `Bid` button, bottom controls, and status label must not overlap incoherently.
...
- If `New Game` is requested after a completed deal, the previous hands, bid values, and post-bidding summary must be cleared, the game phase must return to `notStarted`, any status label text such as `Deal complete` or `Bidding complete` must be hidden, the `Bidding` area must be hidden, a dealer must be selected for the new game, the dealer badge must appear on the selected dealer station, and the squared undealt deck stack must reappear at the very center of the card table.
...
- If all four players pass before any numeric bid is accepted, the app must not display a high-bidding team, bid value, or Tarneeb suit summary, and must automatically begin the next deal with the dealer advanced counterclockwise.
...
- If the South drop-down is used, it must not allow values outside `Pass` and the currently legal numeric bid range through 13.
- If South's Tarneeb suit selector is used, it must not allow values outside spades, clubs, hearts, and diamonds.
- If South selects a numeric bid but no Tarneeb suit, the South `Bid` button must remain disabled and no bid may be committed.
...
- If South changes from `Pass` to a numeric draft bid, a Tarneeb suit must be selected before the South `Bid` button can commit.
- If bidding is in progress and it is not South's turn, the South drop-down must not be visible and the South `Bid` button must remain visible but disabled.
...
- If bidding completes with a numeric high bid, the post-bidding summary must display the correct partnership label, numeric bid, and Tarneeb suit symbol.
- If bidding completes with no numeric high bid, no high-bidding team or Tarneeb suit symbol may be displayed.
...
- If a device is physically rotated, the app should remain in portrait orientation for MVP 005.
...
16. `testNewGameResetsPresentationToLaunchState`
    - Complete a deal, invoke the reset action, and verify the phase returns to `notStarted`, hands are cleared, a dealer is selected, the dealer badge appears on the selected dealer station, the squared undealt deck stack is visible at the very center of the card table, and the status label is hidden.
...
30. `testSouthNumericBidRequiresTarneebSuit`
    - Advance bidding to South's turn, select a numeric bid without a Tarneeb suit, and verify South cannot commit until a suit is selected.

31. `testSouthNumericBidStoresSelectedTarneebSuit`
    - Resolve South's turn with a numeric bid and selected Tarneeb suit, and verify the accepted South bid stores the selected suit alongside the bid value.
...
33. `testSouthBidSubmissionReplacesSelector`
    - Resolve South's turn and verify the selected value replaces the drop-down and suit-selector state and the `Bid` button remains visible but disabled.
...
54. `testPostBiddingSummaryUsesHighBidderTeamAndBid`
    - Resolve bidding with East or West as highest bidder and verify the terminal summary team is `East-West`; resolve bidding with North or South as highest bidder and verify the terminal summary team is `North-South`.
...
57. `testAllPassBiddingStartsAutomaticRedeal`
    - Resolve bidding with all four players passing before any numeric bid is accepted, and verify no high-bidding team or Tarneeb suit symbol is produced, the dealer advances counterclockwise to the player on the previous dealer's right, a fresh deal starts automatically, and the new bidding round begins with all bid entries reset to `--`.
...
20. `testBidTableShowsAllPlayers`
    - Tap `Deal` and verify the bid table shows South, East, North, and West.

21. `testBidTableInitializesWithPendingValues`
    - Tap `Deal` and verify South, East, North, and West initially show `--`.

22. `testSouthBidDropdownAppearsOnlyOnSouthTurn`
    - Verify the South `Bid` button is visible but disabled outside South's turn, then advance bidding to South's turn and verify the South drop-down appears.

23. `testSouthBidDropdownShowsCurrentLegalValues`
    - Advance bidding to South's turn, open the South bid selector, and verify it contains `Pass` plus only legal numeric bids from the current minimum through 13.

24. `testSouthTarneebSuitSelectorAppearsOnlyOnSouthTurn`
    - Verify the South Tarneeb suit selector is absent outside South's turn, then advance bidding to South's turn and verify it offers `♠`, `♣`, `♥`, and `♦`.

25. `testSouthNumericBidRequiresSuitBeforeBidButtonEnables`
    - Advance bidding to South's turn, select a numeric bid without selecting a suit, and verify `Bid` remains disabled until a Tarneeb suit is selected.

26. `testSouthBidSelectionUpdatesTableAfterBidButton`
    - Advance bidding to South's turn, select `Pass` or select both a numeric value and Tarneeb suit, tap `Bid`, and verify the South table entry updates to the bid value and the `Bid` button returns to disabled state unless bidding returns to South.
...
30. `testPostBiddingSummaryDisplaysHighBidTeamBidAndTarneeb`
    - Complete bidding with a numeric high bid and verify the summary displays `East-West` or `North-South`, the bid value, and the expected `Tarneeb` suit symbol.

31. `testAllPassBiddingStartsAutomaticRedeal`
    - Complete bidding with all four players passing before any numeric bid is accepted, verify no post-bidding summary appears, verify the dealer badge advances counterclockwise to the previous dealer's right, and verify a fresh deal animation and bidding round begin automatically.
...
35. `testNoUnsupportedGameplayControlsAreShownAfterDeal`
    - Tap `Deal` and verify unsupported post-bidding trump selection, play-card, trick, and scoring controls are absent; South's bid-time Tarneeb suit selector is allowed only during South's turn.
...
39. `testNewGameButtonResetsToOriginalLaunchState`
    - Complete a deal, tap `New Game`, and verify dealt cards disappear, any `Deal complete` or `Bidding complete` status disappears, the `Bidding` area disappears, a dealer is selected, the dealer badge appears on that station, the squared undealt deck stack returns to the very center of the card table, and the screen remains ready to deal.
...
17. Verify the `Bidding` area contains a table for South, East, North, and West.
...
21. Verify the South bid drop-down allows `Pass` and only currently legal numeric bids through 13.
22. Verify the South Tarneeb suit selector appears on South's turn, offers `♠`, `♣`, `♥`, and `♦`, and is not enabled outside South's turn.
23. Verify selecting a numeric South bid keeps `Bid` disabled until a Tarneeb suit is selected.
24. Verify selecting `Pass` does not require or store a Tarneeb suit.
25. Verify tapping the South `Bid` button replaces the drop-down and suit selector with the selected bid value and returns the button to disabled state unless bidding returns to South.
...
28. Verify accepted numeric bids store the preferred trump/Tarneeb suit alongside the bid value, South numeric bids store South's selected suit, and accepted simulated numeric bids also retain confidence metadata without showing a post-bidding interactive trump selector.
...
34. Verify the post-bidding summary displays the high-bidding team as `East-West` or `North-South`, the bid value, and `Tarneeb` with the preferred suit symbol.
35. Verify an all-pass bidding round shows no post-bidding summary, advances the dealer counterclockwise to the previous dealer's right, and starts a fresh deal automatically.
36. Verify no interactive Tarneeb suit selector appears after the summary.
...
38. Verify tapping `New Game` after a deal returns the app to the launch state with no dealt cards, no `Bidding` area, no post-bidding summary, a selected dealer, a dealer badge on the selected station, and the squared undealt deck stack visible at the very center of the card table.

## 11. Definition of Done

The MVP 005 revision is complete when:
...
- The `Bidding` area contains a table with South, East, North, and West bid values.
...
- The South bid value can be selected from `Pass` and the currently legal numeric bids through 13 using a drop-down list only when it is South's turn.
- The South Tarneeb suit can be selected from `♠`, `♣`, `♥`, and `♦` only when it is South's turn.
- South numeric bids require a selected Tarneeb suit before the `Bid` button can commit the bid.
...
- After South submits a bid, the South drop-down and Tarneeb suit selector are replaced by the selected bid value and the `Bid` button returns to disabled state unless bidding returns to South.
...
- When bidding ends with a numeric high bid, the post-bidding summary displays the high-bidding team as `East-West` or `North-South`, the high bid value, and `Tarneeb` with the preferred suit symbol.
...
- No post-bidding editable Tarneeb suit selection, trick play, scoring, or other post-bidding gameplay is implemented.
- Automated tests verify the core dealing requirements.
- Automated tests verify the allowed bid values, bidding turn order, simulated bid resolution, simulated-player personality adjustments using the normal estimate, South bid and Tarneeb suit submission, and bidding completion rules.
- Automated tests verify post-bidding summary derivation, all-pass automatic redeal behavior, and hand logging.
- Automated or manual visual checks verify the 005 portrait table, rounded-square station, dealer selection, dealer badge, very-centered squared undealt deck stack, bidding area, bottom controls, reset behavior, and card readability changes.
- Automated or manual visual checks verify the deal animation shows 13-card stacks moving from the table center, simulated stations showing hidden backs after arrival, South card faces remaining hidden until all four hands are dealt while any received South stack remains visible as fanned backs, South backs flipping face-up left-to-right over 1.5 seconds, and the center stack disappearing after all 52 cards are dealt.
...
- Exact undealt deck stack offset and rotation are not specified beyond a squared appearance while still representing 52 hidden cards; MVP 005 uses tokenized zero offset and zero rotation.
...
- It is not specified whether replacement deals or all-pass automatic redeals should show an intermediate pre-deal state with the next dealer badge and very-centered squared undealt deck, or whether they remain immediate while dealer rotation is verified through state/test metadata.
...
- It is not specified whether the 52-card undealt deck stack should animate away during deal. MVP 005 only requires that it is present before deal and absent after deal.
- Exact `Bidding` area dimensions, border color, table styling, and spacing below the South station are not specified.
- Exact bid table row order is not specified. Use the existing counterclockwise model order South, East, North, West unless product specifies another order.
- Exact South Tarneeb suit selector control type is not specified beyond allowing one selection from spades, clubs, hearts, and diamonds while South is submitting a numeric bid.
