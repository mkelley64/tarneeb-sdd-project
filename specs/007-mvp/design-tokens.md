# Tarneeb iOS MVP 007 Design Tokens
...
- When a simulated player passes because their partner is currently highest, the
  ...
  or special visual affordance is introduced for MVP 007.
...
| `layout.bidSelector.minimumWidth` | `82pt` | Minimum width for an active South bid chip/action tray item |
...
- An all-pass automatic redeal should reuse the existing `animation.deal.*` timing tokens for its fresh deal animation; no separate automatic-redeal timing tokens are introduced for MVP 007.
