...
### 3.6 MVP 007 Bidding Requirement Changes
...
- When it is South's turn to bid, South's table value should remain `--` until submitted and a bid-chip selector should appear below the table with `Pass` and the currently legal numeric values through `13`; the current legal minimum is `7` when no numeric bid exists, otherwise one more than the current highest numeric bid.
...
- When South selects a bid value and taps `Bid`, the bid-chip selector should disappear, South's table value should be replaced by the selected bid value, and the South `Bid` button should return to its disabled state unless bidding returns to South.
...
- The post-bidding South Tarneeb suit-setting panel should show the high-bidding team, bid value, a `Tarneeb` label, suit chips for `♠`, `♣`, `♥`, and `♦`, and a button labeled `Set`.
...
- The post-bidding summary should show the high-bidding team as `East-West` or `North-South`, the high bid value, and a `Tarneeb` label containing the symbol for the high bidder's preferred suit. The suit symbol should display in a compact white chip, using black for Spades/Clubs and red for Hearts/Diamonds.
...
- Display a post-bidding summary after the `Bidding` area fade completes and any required South suit-setting step is complete, showing the high-bidding team, high bid value, and `Tarneeb` suit symbol when a numeric high bid exists.
...
- Given the post-bidding South Tarneeb suit-setting panel appears, then it displays the high-bidding team, bid value, a `Tarneeb` label, suit chips for spades, clubs, hearts, and diamonds, and a `Set` button.
...
No post-summary editable Tarneeb suit selection, trick-play, scoring, or game-over phase should be implemented for MVP 007. The only interactive Tarneeb suit control in MVP 007 is South's post-bidding suit-setting panel when South is the numeric high bidder.
...
- Simulated bid values must consider basic card strength and partnership context, but do not need to consider score, opponent modeling, or trick-play simulation in MVP 007.
- Accepted simulated numeric bids must store the preferred trump/Tarneeb suit alongside the resolved bid value and retain confidence metadata internally so the high bidder's preferred suit can be shown in the post-bidding summary, without starting post-bidding interactive Tarneeb suit selection in MVP 007.
...
- If no stored preferred Tarneeb suit exists for a numeric highest bid, the state is invalid for MVP 007 because every accepted numeric bid must store a preferred suit.
...
Auction limitations for MVP 007:
...
- The post-bidding summary displays `East-West` or `North-South`, the high bid value, and a `Tarneeb` label with `♠`, `♣`, `♥`, or `♦` in a compact white chip using the proper black/red suit color.
...
- The app must be locked to portrait orientation for MVP 007.
...
- The `Bidding` area, bid table, South bid chips, post-bidding South Tarneeb suit selector, South `Bid` button, and post-bidding `Set` button should be exposed in a way that UI tests can verify visibility, allowed values, enabled/disabled active-turn behavior, selected bid and suit updates, simulated-bid timing, fade/color timing, terminal fade-out, and terminal button disappearance.
...
- If a device is physically rotated, the app should remain in portrait orientation for MVP 007.
...
30. `testPostBiddingSummaryDisplaysHighBidTeamBidAndTarneeb`
    - Complete bidding with a numeric high bid and verify the summary displays `East-West` or `North-South`, the bid value, and the expected `Tarneeb` suit symbol in a compact white chip with black/red suit coloring.
...
34. Verify the post-bidding summary displays the high-bidding team as `East-West` or `North-South`, the bid value, and `Tarneeb` with the preferred suit symbol in a compact white chip using black for Spades/Clubs and red for Hearts/Diamonds.
...
The MVP 007 revision is complete when:
...
- When bidding ends with a numeric high bid by East, North, or West, the post-bidding summary displays the high-bidding team as `East-West` or `North-South`, the high bid value, and `Tarneeb` with the preferred suit symbol.
...
- Exact undealt deck stack offset and rotation are not specified beyond a squared appearance while still representing 52 hidden cards; MVP 007 uses tokenized zero offset and zero rotation.
...
- It is not specified whether the 52-card undealt deck stack should animate away during deal. MVP 007 only requires that it is present before deal and absent after deal.
...
- Exact South Tarneeb suit selector control type is specified as a chip-style post-bidding selector for MVP 007, but exact chip dimensions beyond the tokenized minimums and compact summary symbol padding are not specified.
